  ...
  useEffect(() => {
    if (token && !socket) {
      ...
      newSocket.on('room-success', (data) => {
        console.log('Room success event received:', data);
        setMessage(data.message);
        if (data.message.includes('Left room')) {
          console.log('Handling leave room success');
          // Handle leaving room
          setCurrentRoute('/lobby');
          restartGame();
          setPlayers([]);
      ...
      newSocket.on('room-error', (error) => {
        console.log('Room error event received:', error);
        setMessage(error);
      });
  ...
  const fetchPlayerHistory = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log('Player history set:', data.data);
      } else {
